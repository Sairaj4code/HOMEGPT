...
IMPORTANT
---------
This script expects the tokenizer to have produced:

    tokenizer/vocab_v4.json
    tokenizer/merges_v4.json
    data/train_tokens.pt
    data/val_tokens.pt

The token tensors should contain the FINAL V4 token IDs.
...
FFN_MULTIPLIER = 4
FFN_HIDDEN = int((2 / 3) * FFN_MULTIPLIER * N_EMBD)
...
def main():
    ...
    model = HomeGPT(vocab_size).to(DEVICE)
    ...
    if DEVICE.type == "cuda" and hasattr(torch, "compile"):
        ...
        print("Compile can improve steady-state training speed but has startup cost.")

        # Disabled by default for the first run.
        # We benchmark eager mode first.
        USE_COMPILE = False
    ...
    for step in range(MAX_STEPS):
        ...
        if step % EVAL_INTERVAL == 0 or step == MAX_STEPS - 1:
            ...
            if losses["val"] < best_val:
                ...
                torch.save(
                    checkpoint,
                    CHECKPOINT_DIR / "homegpt_v5_best.pt",
                )
